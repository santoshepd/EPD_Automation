...
# Query to fetch RT and DA Load data
query = """
    SELECT rt.hierarchy_id, rt.date, rt.he, rt.data AS rt_data, da.data AS da_data
    FROM rt_load_ercot rt
    JOIN da_load_ercot da 
...
# Connect to PostgreSQL using psycopg2 and insert/update the data
conn = psycopg2.connect(dbname=db_name, user=db_user, password=db_password, host=db_host, port=db_port)
cursor = conn.cursor()

insert_query = """
    INSERT INTO load_variance_ercot 
...
print("Data inserted into `load_variance_ercot` successfully!")
