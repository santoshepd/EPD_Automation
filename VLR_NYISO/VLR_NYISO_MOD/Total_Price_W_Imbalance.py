...
# Step 4: Calculate total price with imbalance (safely avoid division by 0)
df["data"] = df.apply(lambda row: (row["hedgecostwimbalance"] / row["rt_load"]) 
                      if row["rt_load"] != 0 else 0, axis=1).round(6)
...
try:
    ...
    print("✅ Data inserted into `nyiso_vlr_tot_pricewimbalance` successfully!")

except Exception as e:
    print(f"❌ Error inserting into `nyiso_vlr_tot_pricewimbalance`: {e}")
