...
# Create SQLAlchemy engine for reading
...
# Step 3: Calculate price variance percentage (safe division)
df["data"] = (df["pv_data"] / df["da_data"].replace(0, 1)).round(6)

# Step 4: Select final columns for DB insertion
df = df[[
...
try:
    ...
    cursor.close()
    conn.close()
    print("✅ Price variance percentage inserted into `nyiso_vlr_price_variance_percentage` successfully!")

except Exception as e:
    print(f"❌ Error inserting into price variance percentage table: {e}")
