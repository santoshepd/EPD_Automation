# Total_Price_W_Imbalance.py - autogenerated file
import os
...
query = """
    ...
    FROM miso_vlr_tot_hedgecostwimbalance hcw
    JOIN miso_vlr_rt_load rt_load
    ON hcw.hierarchy_id = rt_load.hierarchy_id 
    AND hcw.date = rt_load.date 
    AND hcw.he = rt_load.he
    WHERE hcw.hierarchy_id BETWEEN 1 AND 19
"""
...
# # Step 6: Insert into miso_vlr_tot_pricewimbalance
try:
    conn = psycopg2.connect(**DB_PARAMS)
    cursor = conn.cursor()

    insert_query = """
        INSERT INTO miso_vlr_tot_pricewimbalance 
    ...
    print("✅ Data inserted into `miso_vlr_tot_pricewimbalance` successfully!")

except Exception as e:
    print(f"❌ Error inserting into `miso_vlr_tot_pricewimbalance`: {e}")
