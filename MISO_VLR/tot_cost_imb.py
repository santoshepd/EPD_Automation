...
# Step 1: Fetch Load Variance and RT_LMPS Data for miso
query = """
    SELECT lv.hierarchy_id, lv.date, lv.he, 
           lv.data AS load_variance, rt_lmps.data AS rt_lmps
    FROM miso_vlr_load_variance lv
    JOIN miso_vlr_rt_lmps rt_lmps 
...
# Step 6: Insert into `miso_vlr_tot_costofimbalance`
try:
    conn = psycopg2.connect(**DB_PARAMS)
    cursor = conn.cursor()

    insert_query = """
        INSERT INTO miso_vlr_tot_costofimbalance 
    ...
    cursor.close()
    conn.close()
    print("✅ Data inserted into `miso_vlr_tot_costofimbalance` successfully!")

except Exception as e:
    print(f"❌ Error inserting into `miso_vlr_tot_costofimbalance`: {e}")
